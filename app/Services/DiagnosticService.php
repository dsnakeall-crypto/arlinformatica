...
class DiagnosticService
{
    public function run(): array
    {
        ...
        try {
            ...
            $database = 'error';
            $databaseError = 'Falha ao consultar o banco.';
        }
        ...
        return [
            ...
            'web_push' => ['status' => $this->pushConfigured() ? 'ok' : 'warning', 'configured' => $this->pushConfigured(), 'library_available' => class_exists(WebPush::class)],
    ...
    private function storageCheck(): array
    {
        ...
        $probe = '.diagnostic-'.bin2hex(random_bytes(6));
        try {
            File::put("$root/$probe", 'ok');
            $writable = File::get("$root/$probe") === 'ok';
        } catch (Throwable) {
            $writable = false;
        } finally {
            File::delete("$root/$probe");
        }
    ...
    {
        return is_dir($path) ? collect(File::allFiles($path))->sum(fn ($file) => $file->getSize()) : 0;
    }

    private function pushConfigured(): bool
    {
        return filled(config('webpush.subject')) && filled(config('webpush.public_key')) && filled(config('webpush.private_key')) && class_exists(WebPush::class);
    }
}
