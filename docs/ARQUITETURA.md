...
A preferência de experiência (`automatic`, `desktop` ou `mobile`) é local ao navegador/dispositivo e não integra snapshots empresariais. O Painel e a Mesa consomem o endpoint dedicado `/orders/desk`, sem paginação; o Painel apresenta a tabela compartilhada sem filtros e a Lista de OS mantém paginação server-side. A configuração de backup automático passou do `.env` para `settings`, com auditoria; o cron continua sem worker permanente e lê essa preferência persistida a cada execução do scheduler.
