...
function tagSettingsForm(form:HTMLFormElement){let current='';for(const child of Array.from(form.children) as HTMLElement[]){if(child.tagName==='H2'){const label=text(child);current=label==='Dados da Empresa'?'company':label==='Identidade Visual'?'identity':label==='Documentos'?'documents':label==='Garantia Geral'?'warranty':label.startsWith('Pós-Venda')?'hidden':label.startsWith('Layout')?'hidden':current}if(current)child.dataset.arlSettingsSection=current;if(child.classList.contains('notice'))child.dataset.arlSettingsSection='save-feedback';if(child.classList.contains('actions'))child.dataset.arlSettingsSection='save-actions'}}
