...
function RefundModal({ order, maximum, onClose, onSaved }: any) {
  ...
  return <div className="arl-od-modal"><section className="arl-od-card" role="dialog" aria-modal="true" aria-label={`Registrar estorno da OS #${order.number}`}><h2>Registrar estorno</h2><p>O valor original da OS será preservado. A devolução será uma saída financeira na data de hoje.</p><TextField label="Valor devolvido (R$)" value={value} onChange={(e: any) => setValue(e.target.value)} required/><label>Motivo obrigatório<textarea value={reason} onChange={(e) => setReason(e.target.value)}/></label><label>Forma de devolução<select value={method} onChange={(e) => setMethod(e.target.value)}>{[['pix','Pix'],['cash','Dinheiro']].map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label><div className="notice">Disponível para estorno: <b>{money(maximum)}</b></div>{error&&<div className="alert">{error}</div>}<div className="arl-od-actions"><button type="button" onClick={onClose}>Cancelar</button><button type="button" className="primary" disabled={busy} onClick={save}>{busy?'Registrando…':'Confirmar estorno'}</button></div></section></div>;
}
