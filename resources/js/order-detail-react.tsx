...
function InterruptionModal({ order, onClose, onSaved }: any) {
  ...
  return <div className="arl-status-modal"><section className="arl-status-modal-card" role="dialog" aria-modal="true" aria-label="Interromper OS">
    ...
    <label>Motivo da interrupção<textarea spellCheck={true} value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Ex.: cliente pediu pausa, aguardando decisão, atendimento suspenso..."/></label>
    <label>O que já foi feito no equipamento? *<textarea spellCheck={true} aria-label="O que já foi feito no equipamento" value={workDone} onChange={(e) => setWorkDone(e.target.value)} placeholder='Se nada foi feito, escreva "Nada".'/></label>
...
function RefundModal({ order, maximum, onClose, onSaved }: any) {
  ...
  return <div className="arl-od-modal"><section className="arl-od-card" role="dialog" aria-modal="true" aria-label={`Registrar estorno da OS #${order.number}`}><h2>Registrar estorno</h2><p>O valor original da OS será preservado. A devolução será uma saída financeira na data de hoje.</p><TextField label="Valor devolvido (R$)" value={value} onChange={(e: any) => setValue(e.target.value)} required/><label>Motivo obrigatório<textarea spellCheck={true} value={reason} onChange={(e) => setReason(e.target.value)}/></label><label>Forma de devolução<select value={method} onChange={(e) => setMethod(e.target.value)}>{[['pix','Pix'],['cash','Dinheiro']].map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label><div className="notice">Disponível para estorno: <b>{money(maximum)}</b></div>{error&&<div className="alert">{error}</div>}<div className="arl-od-actions"><button type="button" onClick={onClose}>Cancelar</button><button type="button" className="primary" disabled={busy} onClick={save}>{busy?'Registrando…':'Confirmar estorno'}</button></div></section></div>;
}
...
function FinalizationBox({ order, reload, openSignal = 0, finalReport, setFinalReport, persistPendingChanges, onFinalReportDirty }: any) {
  ...
  const finish = async () => {
    if (isPaid && !paymentMethod) { setError('Escolha a forma de pagamento.'); return; }
    setBusy(true); setError('');
    try {
...
export default function OrderDetailPage({ id, back, readOnly = false, reopenOnLoad = false, onEdit, onDirtyChange, onOpenClientHistory }: Props & { readOnly?: boolean }) {
  ...
  const openClosingReference = () => { setClosingValue(order.closing_reference_cents != null ? (order.closing_reference_cents / 100).toFixed(2).replace('.', ',') : ''); setClosingError(''); setClosingOpen(true); };
  const saveClosingReference = async () => { const amount = Math.round(Number(closingValue.replace(',', '.')) * 100); if (!Number.isFinite(amount) || amount <= 0) return setClosingError('Informe um valor combinado maior que zero.'); setClosingBusy(true); setClosingError(''); try { await api(`/orders/${order.id}/closing-reference`, { method: 'PATCH', body: JSON.stringify({ amount_cents: amount }) }); setClosingOpen(false); await load(); } catch (reason: any) { setClosingError(reason.message || 'Não foi possível marcar a OS.'); } finally { setClosingBusy(false); } };
  const clearClosingReference = async () => { setClosingBusy(true); setClosingError(''); try { await api(`/orders/${order.id}/closing-reference`, { method: 'DELETE' }); setClosingOpen(false); await load(); } catch (reason: any) { setClosingError(reason.message || 'Não foi possível desmarcar a OS.'); } finally { setClosingBusy(false); } };
  const closingMarker = order.closing_reference_cents != null ? <span className="arl-closing-marker arl-order-closing-marker">⚑ Fechar · {money(order.closing_reference_cents)}{order.closing_marked_by?.name && order.closing_marked_at ? ` — marcado por ${order.closing_marked_by.name} em ${new Date(order.closing_marked_at).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}` : ''}</span> : null;
  const closingDialog = closingOpen ? <div className="modal"><section className="modal-card arl-closing-dialog" role="dialog" aria-modal="true" aria-label="Precisa fechar"><h2>Precisa fechar</h2><p>Registre somente o valor combinado em campo. Isso não altera itens, total ou financeiro.</p><label className="field"><span>Valor combinado (R$) *</span><input autoFocus inputMode="decimal" value={closingValue} onChange={(event) => setClosingValue(event.target.value)} /></label>{closingError && <div className="alert">{closingError}</div>}<div className="actions"><button type="button" disabled={closingBusy} onClick={() => setClosingOpen(false)}>Cancelar</button>{order.closing_reference_cents != null && <button type="button" disabled={closingBusy} onClick={() => void clearClosingReference()}>Desmarcar</button>}<button type="button" className="primary" disabled={closingBusy} onClick={() => void saveClosingReference()}>{closingBusy ? 'Salvando…' : 'Confirmar'}</button></div></section></div> : null;
  ...
  if (readOnly) return <div data-arl-order-detail-react="1" data-mobile-read-only="1" className="arl-mobile-read-only">
    <header className="arl-mobile-read-only-header"><button type="button" className="arl-back-button" onClick={back} aria-label="Voltar para OS abertas">←</button><div><span>ORDEM DE SERVIÇO</span><h1>OS #{order.number}</h1><span className="arl-order-markers">{order.attendance_type === 'external'&&<span className="arl-external-marker">Externo</span>}{reopened&&<span className="arl-reopened-marker arl-order-reopened-marker">Reaberta</span>}{closingMarker}</span>{interrupted&&<span className="arl-reopened-marker arl-interrupted-marker arl-order-reopened-marker">Interrompida</span>}</div></header>
  ...
  return <div data-arl-order-detail-react="1" className="arl-order-detail-page">
    <header className="arl-order-sticky-header">
      <div className="arl-order-header-main">
        ...
        <div className="arl-order-header-identity"><span className="arl-eyebrow">ORDEM DE SERVIÇO</span><h1>OS #{order.number}</h1><span className="arl-order-markers">{order.attendance_type === 'external'&&<span className="arl-external-marker">Externo</span>}{reopened&&<span className="arl-reopened-marker arl-order-reopened-marker">Reaberta</span>}{closingMarker}</span>{interrupted&&<span className="arl-reopened-marker arl-interrupted-marker arl-order-reopened-marker">Interrompida</span>}</div>
      </div>
      <div className="arl-order-quick-actions arl-order-header-actions">
        ...
        {canAdminister && operational && <button type="button" data-order-action="closing-reference" aria-label="Precisa fechar" onClick={openClosingReference}><span>⚑</span><span>Precisa fechar</span></button>}
    ...
    <section className="panel arl-intake-card" aria-labelledby="arl-intake-title">
      ...
      <div className="arl-intake-grid">
        ...
        <section className="arl-intake-field arl-intake-equipment-field"><h3>Equipamento</h3><div><p className="arl-intake-equipment-name"><strong>{order.equipment_description || 'Equipamento não informado'}</strong></p><div className="arl-intake-equipment-details"><h4>Fabricante / Modelo / Acessórios</h4><p>{order.equipment_details || 'Não informado'}</p></div></div></section>
