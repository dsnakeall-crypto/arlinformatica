...
function FinalizationBox({ order, reload, openSignal = 0, finalReport, setFinalReport, persistPendingChanges, onFinalReportDirty }: any) {
  ...
  const [open, setOpen] = useState(false), [discount, setDiscount] = useState('0'), [items, setItems] = useState<any[]>(seeded), [catalog, setCatalog] = useState<any[]>([]), [budgets, setBudgets] = useState<any[]>([]), [sourceBudgetId, setSourceBudgetId] = useState<number | null>(null), [showItemWarranties, setShowItemWarranties] = useState(false), [isPaid, setIsPaid] = useState(false), [paymentMethod, setPaymentMethod] = useState(''), [error, setError] = useState(''), [busy, setBusy] = useState(false);
  ...
  const finish = async () => {
    ...
    try {
      ...
      const payload: any = { technical_report: finalReport.trim(), discount_cents: disc, approved_budget_id: sourceBudgetId, photo_ids: (order.photos || []).map((row: any) => row.id), show_item_warranties: showItemWarranties, is_paid: isPaid, payment_method: isPaid ? paymentMethod : null };
  ...
  return <div data-arl-order-detail-react="1" className="arl-order-detail-page">
    <header className="arl-order-sticky-header">
      <div className="arl-order-header-main">
        ...
        <div className="arl-order-header-identity"><span className="arl-eyebrow">ORDEM DE SERVIÇO</span><h1>OS #{order.number}</h1><span className="arl-order-markers">{reopened&&<span className="arl-reopened-marker status-paid arl-order-reopened-marker">Reaberta</span>}{closingMarker}</span>{interrupted&&<span className="arl-reopened-marker arl-interrupted-marker arl-order-reopened-marker">Interrompida</span>}</div>
      </div>
