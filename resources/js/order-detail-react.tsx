...
function BudgetBox({ order, role, openSignal = 0 }: any) {
  ...
  useEffect(() => { if (openSignal && order.status !== 'completed') setOpen(true); }, [openSignal, order.status]);
  ...
  return <section className="wide"><div className="section-title"><h2>Orçamentos</h2>{order.status !== 'completed' && <button className="primary" data-arl-quick-source="budget" onClick={() => setOpen(true)}><Plus/>Gerar orçamento</button>}</div>
    ...
    {error && !open && <div className="alert">{error}</div>}{list.length ? list.map((budget) => <p key={budget.id}>Revisão {budget.revision} · {budget.status} · R$ {(budget.total_cents / 100).toFixed(2)} · <a target="_blank" rel="noreferrer" href={`/api/orders/${order.id}/budgets/${budget.revision}/pdf`}>Abrir PDF</a> {budget.status === 'draft' && <button onClick={() => api(`/orders/${order.id}/budgets/${budget.revision}/status`, { method: 'PATCH', body: JSON.stringify({ status: 'sent' }) }).then(load)}>Marcar enviado</button>}{budget.status === 'sent' && <><button onClick={() => api(`/orders/${order.id}/budgets/${budget.revision}/status`, { method: 'PATCH', body: JSON.stringify({ status: 'approved' }) }).then(load)}>Aprovar orçamento</button><button onClick={() => api(`/orders/${order.id}/budgets/${budget.revision}/status`, { method: 'PATCH', body: JSON.stringify({ status: 'refused' }) }).then(load)}>Recusar</button></>}{['Master', 'Administrador'].includes(role) && !budget.used_in_finalization && <button type="button" onClick={() => void remove(budget)}>Excluir orçamento</button>}</p>) : <p>Nenhum orçamento criado.</p>}
  </section>;
}
