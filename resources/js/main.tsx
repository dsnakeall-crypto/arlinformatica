      ...
      <table>
        ...
        <tbody>{items.map((o: Order) => {
          ...
          return (
            <tr className={`order-row${reopened ? " order-row-reopened" : ""}${external ? " order-row-external" : ""}`} key={o.id}>
              <td><b>#{o.number}</b></td>
              <td><span className="order-customer"><strong>{o.client.name}</strong><span className="arl-order-markers">{external && <span className="arl-external-marker" aria-label="Atendimento externo">Externo</span>}{reopened && <span className="arl-reopened-marker" aria-label="OS reaberta">Reaberta</span>}{o.closing_reference_cents != null && <span className="arl-closing-marker" aria-label="OS precisa fechar">⚑ Fechar · {money(o.closing_reference_cents)}</span>}</span>{interrupted && <span className="arl-reopened-marker arl-interrupted-marker" aria-label="OS interrompida">Interrompida</span>}</span></td>
              <td><span className="order-device" title={o.equipment_description || undefined}><Box aria-hidden="true" />{o.equipment_description || ""}</span></td>
