...
type Client = { id: number; name: string; nickname?: string | null; phone: string; document: string; [key: string]: any };
...
export default function NewOrderClientPicker({ selected, onSelect }: { selected?: Client; onSelect: (client: Client) => void }) {
  ...
  return <div className="arl-client-search">
    <label><span className="arl-search-icon">⌕</span><input type="search" autoComplete="off" placeholder="Buscar por nome, apelido, telefone ou CPF/CNPJ" value={query} onChange={event => { setQuery(event.target.value); setSearching(true); }}/><span className="arl-client-search-state">{searching ? message : ''}</span></label>
    <div className="arl-client-results">{searching && results.map(client => <button type="button" key={client.id} data-id={client.id} onClick={() => { onSelect(client); setQuery(client.name); setSearching(false); setResults([]); }}><b>{client.name}</b>{client.nickname && <small className="arl-client-nickname">{client.nickname}</small>}<small>{phone(client.phone)} · {documentLabel(client.document)}{client.city ? ` · ${client.city}/${client.state}` : ''}</small></button>)}</div>
  </div>;
}
