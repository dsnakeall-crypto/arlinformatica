...
<td class="brand">@if(!empty($company['logo']))<img src="{{ storage_path('app/private/'.$company['logo']) }}">@else<div class="brand-fallback">ARL</div><div class="green" style="font-size:7px;letter-spacing:2px">INFORMÁTICA</div>@endif</td>
<td class="company"><strong>{{ $company['company_name'] }}</strong>@if(($company['show_company_address'] ?? '1') !== '0')<div>{{ $company['street'] }}{{ filled($company['number'] ?? null) ? ', '.$company['number'] : '' }}{{ filled($company['district'] ?? null) ? ' — '.$company['district'] : '' }}</div><div>{{ $company['city'] }}/{{ $company['state'] }}{{ filled($company['postal_code'] ?? null) ? ' · CEP '.$company['postal_code'] : '' }}</div>@endif @if(($company['show_company_document'] ?? '1') !== '0' && filled($company['cnpj'] ?? null))<div>CNPJ: {{ $company['cnpj'] }}</div>@endif<div>{{ $company['phone'] }}{{ filled($company['email'] ?? null) ? ' · '.$company['email'] : '' }}</div>@if(filled($company['instagram'] ?? null))<div>{{ $company['instagram'] }}</div>@endif</td>
