...
<p><b>Equipamento:</b> {{ data_get($order, 'snapshot.equipment.name', 'Conforme OS') }}@if(filled(data_get($order, 'snapshot.equipment.details')))<br><b>Fabricante / Modelo / Acessórios:</b> {{ data_get($order, 'snapshot.equipment.details') }}@endif<br><b>Problema relatado:</b> {{ $order['reported_problem'] }}</p>
