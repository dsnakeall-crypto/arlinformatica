...
class StageThreeTest extends TestCase
{
    ...
    {
        parent::setUp(); $this->seed(DatabaseSeeder::class); $role=DB::table('roles')->where('name','Master')->value('id');
    ...
    {
        Storage::fake('local'); $png=base64_decode('iVBORw0KGgoAAAANSUhEUgAAAAIAAAABCAYAAAD0In+KAAAADUlEQVQIHWP4z8DwHwAFgAI/ScLQAAAAAElFTkSuQmCC');
        $response=$this->actingAs($this->user)->post('/api/settings/logo',['logo'=>UploadedFile::fake()->createWithContent('logo.png',$png)]);
        $response->assertCreated(); foreach(['app','menu','term','a4','budget','report'] as $variant) Storage::disk('local')->assertExists($response->json($variant));
    }

    public function test_budget_total_revision_warranty_snapshot_and_historical_document(): void
    {
        Storage::fake('local'); $payload=['diagnosis'=>'SSD danificado','proposal'=>'Substituir SSD','validity_days'=>7,'items'=>[['description'=>'SSD Kingston','quantity'=>2,'unit_price_cents'=>25000,'warranty_enabled'=>true,'warranty_term'=>1,'warranty_unit'=>'years'],['description'=>'Formatação','quantity'=>1,'unit_price_cents'=>10000,'warranty_enabled'=>false]]];
        ...
        $warranty=json_decode(DB::table('budget_items')->where('budget_id',$first->json('budget.id'))->first()->warranty_snapshot,true); $this->assertSame(['enabled'=>true,'term'=>1,'unit'=>'years'],$warranty);
        DB::table('settings')->updateOrInsert(['key'=>'company_name'],['value'=>'Empresa Nova','created_at'=>now(),'updated_at'=>now()]);
        $snapshot=json_decode(DB::table('generated_documents')->where(['service_order_id'=>$this->order->id,'type'=>'budget','revision'=>1])->value('snapshot'),true); $this->assertSame('ARL Informática',$snapshot['company']['company_name']);
    }

    public function test_term_uses_order_snapshot_and_is_not_regenerated(): void
    {
        Storage::fake('local'); $this->actingAs($this->user)->get("/api/orders/{$this->order->id}/term")->assertOk(); DB::table('service_order_snapshots')->where('service_order_id',$this->order->id)->update(['term_text'=>'Texto novo']);
        $this->actingAs($this->user)->get("/api/orders/{$this->order->id}/term")->assertOk(); $snapshot=json_decode(DB::table('generated_documents')->where(['service_order_id'=>$this->order->id,'type'=>'term'])->value('snapshot'),true); $this->assertSame('Termo histórico',$snapshot['snapshot']['term_text']);
    }
}
